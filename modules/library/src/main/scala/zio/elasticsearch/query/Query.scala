...
object Query {
  sealed trait BoolQuery[S] extends Query[S] with HasBoost[BoolQuery[S]] {
    def filter(queries: Query[S]*): BoolQuery[S]

    def must(queries: Query[S]*): BoolQuery[S]

    def mustNot(queries: Query[S]*): BoolQuery[S]

    def should(queries: Query[S]*): BoolQuery[S]
  }

  private[elasticsearch] final case class Bool[S](
    filter: List[Query[S]],
    must: List[Query[S]],
    mustNot: List[Query[S]],
    should: List[Query[S]],
    boost: Option[Double]
  ...
  sealed trait ExistsQuery[S] extends Query[S]
  ...
  sealed trait MatchQuery[S] extends Query[S]
  ...
  sealed trait MatchAllQuery extends Query[Any] with HasBoost[MatchAllQuery]
  ...
  sealed trait NestedQuery[S] extends Query[S] with HasIgnoreUnmapped[NestedQuery[S]] with HasScoreMode[NestedQuery[S]]
  ...
  sealed trait TermQuery[S] extends Query[S] with HasBoost[TermQuery[S]] with HasCaseInsensitive[TermQuery[S]]
